...
const SwirlOperatingSystemView: React.FC = () => {
  ...
  return <section className="py-20 md:py-28 bg-gradient-to-b from-white via-blue-50/30 to-white relative overflow-hidden">
      {/* Subtle background */}
        ...
        <div className="max-w-6xl mx-auto animate-on-scroll animate-delay-300">
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-3 md:grid-cols-5 gap-3 md:gap-4 mb-3 md:mb-4">
              {modules.slice(0, 5).map(module => <ModuleCard key={module.id} module={module} isActive={activeModule === module.id} onHover={setActiveModule} />)}
            </div>

            {/* Central Core */}
            <div className="flex justify-center my-6 md:my-8">
              <div className="relative group cursor-pointer" onMouseEnter={() => setActiveModule('core')} onMouseLeave={() => setActiveModule(null)}>
                {/* Glow rings */}
                ...
                {/* Rotating ring */}
                <div className="absolute inset-[-8px] rounded-full border-2 border-dashed border-primary/30 animate-spin" style={{
                animationDuration: '20s'
              }} />
                <div className="absolute inset-[-16px] rounded-full border border-primary/10 animate-spin" style={{
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mt-3 md:mt-4">
              {modules.slice(5).map(module => <ModuleCard key={module.id} module={module} isActive={activeModule === module.id} onHover={setActiveModule} />)}
            </div>
          </div>
        </div>
...
const ModuleCard: React.FC<ModuleCardProps> = ({
  ...
  return <div className={`
      ...
      `} onMouseEnter={() => onHover(module.id)} onMouseLeave={() => onHover(null)}>
      {/* Glow effect */}
      <div className={`
      ...
      `}>
        <div className={`
          ...
          ${isActive ? `bg-gradient-to-br ${module.color} text-white shadow-lg` : 'bg-primary/10 text-primary group-hover:bg-primary/20'}
        `}>
        ...
        {/* Connection pulse */}
        {isActive && <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-green-500 animate-ping" />}
      </div>
    </div>;
};
