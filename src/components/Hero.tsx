...
const MENU_CATEGORIES = [
  {
    ...
    items: [
      ...
      { name: 'Vanilla Affogato', price: 25 },
      { name: 'Matcha Latte', price: 22 },
    ],
  },
  {
    name: 'Manakish',
    items: [
      ...
      { name: 'Cheese Manakish', price: 14 },
      { name: 'Labneh Manakish', price: 12 },
    ],
  },
  {
    name: 'Hot Drinks',
  ...
  },
  {
    name: 'Food',
    items: [
      { name: 'Texas BBQ Sandwich', price: 35 },
      { name: 'Club Sandwich', price: 30 },
      { name: 'Caesar Salad', price: 28 },
      { name: 'Grilled Chicken Wrap', price: 32 },
    ],
  },
];
...
const PREDEFINED_DISCOUNTS = [
  { name: 'Rahbar Disc', description: 'First order', percent: 10 },
  { name: 'Khalid disc', description: 'recurring customer', percent: 10 },
  { name: 'Rohit', description: '', percent: 10 },
  { name: 'VALENTINES10', description: 'Valentines Day Offer', percent: 10 },
  { name: 'Tasteful Treats', description: '', percent: 15 },
  { name: 'BRUNCH10', description: 'Brunch 10', percent: 10 },
];

const PAYMENT_METHODS = [
  { name: 'Master Card', type: 'CREDIT CARD', icon: CreditCard },
  { name: 'Visa', type: 'CREDIT CARD', icon: CreditCard },
  { name: 'CASH', type: 'CASH', icon: Banknote },
  { name: 'Aggregator Payment', type: 'MOBILE PAYMENT', icon: Smartphone },
  { name: 'Swirl Credit', type: 'RESTAURANT CREDIT', icon: CircleDollarSign, highlight: true },
  { name: 'SPLIT PAYMENT', type: '', icon: SplitSquareVertical },
];

// ============ MODAL OVERLAY ============
const ModalOverlay = ({ children, onClose, wide }: { children: React.ReactNode; onClose: () => void; wide?: boolean }) => (
  <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm rounded-2xl" onClick={onClose}>
    <div
      className={`bg-white rounded-2xl shadow-2xl ${wide ? 'max-w-lg w-[95%]' : 'max-w-md w-[92%]'} max-h-[85%] overflow-auto border border-gray-200 animate-scale-in`}
...
const Hero: React.FC = () => {
  ...
  // Add Items staging
  const [stagingItem, setStagingItem] = useState<{ name: string; price: number } | null>(null);
  ...
  const [appliedDiscount, setAppliedDiscount] = useState<{ type: string; value: number; reason: string } | null>(null);
    ...
    return (
      <div className="relative">
        ...
        {isHighlighted && stepInfo && (
          <div className="absolute -top-16 left-1/2 -translate-x-1/2 z-30 w-52 pointer-events-none animate-fade-in">
  ...
  const confirmAddItem = useCallback(() => {
    ...
    setOrderItems((prev) => {
      ...
      if (existing && !stagingNote) {
        return prev.map((i) => (i.id === existing.id ? { ...i, qty: i.qty + stagingQty, isNew: true } : i));
      }
      return [...prev, { id: Date.now().toString(), name: stagingItem.name, qty: stagingQty, price: stagingItem.price, note: stagingNote || undefined, isNew: true }];
    });
    setStagingItem(null);
    setStagingQty(1);
    setStagingNote('');
  ...
  const updateQty = useCallback((id: string, delta: number) => {
    setOrderItems((prev) => prev.map((i) => (i.id !== id ? i : { ...i, qty: Math.max(1, i.qty + delta) })));
  }, []);
  ...
  const handlePrintKOT = useCallback(() => {
    ...
    setOrderItems((prev) => prev.map((i) => ({ ...i, isNew: false })));
    setKotSent(true);
  }, [orderItems]);

  const applyPredefinedDiscount = useCallback((disc: typeof PREDEFINED_DISCOUNTS[0]) => {
    setAppliedDiscount({ type: 'Percentage', value: disc.percent, reason: disc.name });
    closeModal();
  }, [closeModal]);
  ...
  const applySpotDiscount = useCallback(() => {
    ...
    setAppliedDiscount({ type: discountType, value: val, reason: discountReason });
    closeModal();
              ...
              <div className="flex gap-5">
                <div className="w-32 flex-shrink-0 space-y-1">
                  {MENU_CATEGORIES.map((cat, idx) => (
                    <button
                      key={cat.name}
                      onClick={() => setMenuCategory(idx)}
                      className={`w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                        idx === menuCategory ? 'bg-primary/10 text-primary border-l-3 border-primary' : 'text-gray-600 hover:bg-gray-50'
                      }`}
                    >
                      {cat.name}
                    </button>
                ...
                <div className="flex-1">
                  {/* Staging item form */}
                  {stagingItem ? (
                    <div className="border-2 border-primary/20 rounded-xl p-4 bg-primary/5 animate-fade-in">
                      ...
                      <div className="mb-4">
                        ...
                        <input
                          className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-primary/30 focus:outline-none"
                          placeholder="e.g. no sugar, extra hot, almond milk..."
                          value={stagingNote}
                      ...
                      <button
                        ...
                        className="w-full py-2.5 bg-gray-900 text-white text-sm font-semibold rounded-xl hover:bg-gray-800 transition-colors flex items-center justify-center gap-2"
                      >
          ...
          <ModalOverlay onClose={closeModal}>
            <div className="p-5">
              ...
              <div className="space-y-2.5 mb-4">
                {orderItems.map((item) => (
                  <div key={item.id} className="border border-gray-200 rounded-xl p-3 flex items-center justify-between gap-2">
            ...
            <div className="p-5 bg-gray-100 font-mono text-sm">
              <div className="bg-white p-5 rounded-xl shadow-inner border border-gray-300">
                <div className="text-center mb-3">
                  ...
                  {newItems.length > 0 && !kotSent && (
                    <span className="inline-block mt-1 text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-sans font-medium">
                      New Items Only
                    </span>
                  )}
                </div>
                ...
                <div className="border-t border-dashed border-gray-400 my-3" />
                {!kotSent ? (
                  <button onClick={handlePrintKOT} className="w-full py-2.5 bg-green-600 text-white rounded-xl font-bold text-sm hover:bg-green-700 flex items-center justify-center gap-2 transition-colors">
                    <ChefHat className="w-4 h-4" /> Send to Kitchen
                  </button>
                ) : (
                  <div className="text-center text-green-600 font-bold flex items-center justify-center gap-2 py-2 font-sans">
          ...
          <ModalOverlay onClose={closeModal} wide>
            <div className="p-5 md:p-6">
              ...
              <div className="flex gap-2 mb-5">
                <button
                  onClick={() => setDiscountTab('predefined')}
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${discountTab === 'predefined' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  Pre-Defined Discount
                </button>
                <button
                  onClick={() => setDiscountTab('spot')}
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${discountTab === 'spot' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  Spot Discount
                </button>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    {PREDEFINED_DISCOUNTS.map((disc) => {
                      ...
                      return (
                        <button
                          key={disc.name}
                          onClick={() => applyPredefinedDiscount(disc)}
                          className="p-4 border border-gray-200 rounded-xl text-left hover:border-primary/30 hover:shadow-sm transition-all"
                        >
                ...
                <div className="border border-gray-200 rounded-xl p-5">
                  <div className="mb-4">
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-primary/30 focus:outline-none bg-white"
                    >
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-primary/30 focus:outline-none"
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-primary/30 focus:outline-none"
                      placeholder="e.g. friends, family, loyalty"
                    />
                  </div>
                  ...
                  {spotDiscountCalc > 0 && (
                    <p className="text-sm text-primary mb-4">Discount amount: {formatAmount(spotDiscountCalc)}</p>
                  )}
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      onClick={applySpotDiscount}
                      className="flex-1 py-2.5 bg-gray-900 text-white rounded-xl text-sm font-semibold hover:bg-gray-800 transition-colors flex items-center justify-center gap-2"
            ...
            <div className="p-5">
              ...
              <div className="grid grid-cols-3 gap-2.5 max-h-72 overflow-auto">
                {TABLES.map((table) => (
                  <button
                    key={table.id}
                    onClick={() => { setCurrentTable(table.name); closeModal(); }}
          ...
          <ModalOverlay onClose={closeModal}>
            <div className="p-5">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => { if (splitItems.size > 0) { setOrderItems((prev) => prev.filter((i) => !splitItems.has(i.id))); closeModal(); } }}
                  className="flex-1 py-2.5 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary/90 disabled:opacity-50"
                  disabled={splitItems.size === 0}
                >
        ...
        return (
          <ModalOverlay onClose={closeModal}>
            <div className="p-5">
              ...
              <div className="flex gap-3">
                <button onClick={closeModal} className="flex-1 py-2.5 border border-gray-300 rounded-xl text-sm font-medium hover:bg-gray-50">Cancel</button>
                <button onClick={() => { setFulfillment('Takeaway'); closeModal(); }} className="flex-1 py-2.5 bg-gray-900 text-white rounded-xl text-sm font-semibold hover:bg-gray-800">
                  Move to Takeaway
                </button>
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="space-y-2.5 mb-4">
                    {PAYMENT_METHODS.map((m) => (
                      <button
                        key={m.name}
                        onClick={() => handleSettleBill(m.name)}
                        className={`w-full p-4 border rounded-xl text-left flex items-center gap-3 transition-all hover:shadow-sm ${m.highlight ? 'border-primary/30 bg-primary/5' : 'border-gray-200 hover:border-gray-300'}`}
                      >
              ...
              {!paymentSuccess ? (
                <>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-4">
                    {[
                      { icon: Banknote, label: 'Cash', color: 'bg-green-600' },
                      { icon: CreditCard, label: 'Card', color: 'bg-primary' },
                      { icon: Wallet, label: 'Wallet', color: 'bg-gray-700' },
                    ].map((m) => (
                      <button key={m.label} onClick={() => handleSettleBill(m.label)} className={`${m.color} text-white rounded-xl p-3.5 flex flex-col items-center gap-1.5 hover:opacity-90 transition-opacity`}>
                ...
                <div className="p-4 md:p-5 border-b md:border-b-0 md:border-r border-gray-100">
                  ...
                  <div className="space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => setOrderStatus(orderStatus === 'IN_PROGRESS' ? 'CONFIRMED' : 'IN_PROGRESS')}
                        className="px-3 py-2.5 text-xs font-medium text-orange-600 border border-orange-200 rounded-xl hover:bg-orange-50 transition-colors"
                      >
                        Change Status
                      </button>
                ...
                <div className="p-4 md:p-5 bg-gray-50/50">
                  ...
                  <div className="bg-white rounded-xl border border-gray-100 p-4 mb-4">
                    ...
                    <div className="space-y-0.5 mb-3">
                      ...
                      {orderItems.map((item) => (
                        <div key={item.id} className="grid grid-cols-12 text-xs text-gray-700 py-0.5">
