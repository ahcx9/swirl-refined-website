...
const DeliveryComingSoon = () => {
  ...
  const deliveryApps = [
    { name: "UberEats", color: "from-green-400 to-green-600", icon: "🚗" },
    { name: "DoorDash", color: "from-red-400 to-red-600", icon: "🏠" },
    { name: "Grubhub", color: "from-orange-400 to-orange-600", icon: "🍔" },
    { name: "Postmates", color: "from-blue-400 to-blue-600", icon: "📦" },
    { name: "Zomato", color: "from-pink-400 to-pink-600", icon: "🍽️" }
  ];
  ...
  return (
    <section 
      ...
      className="relative py-24 overflow-hidden bg-gradient-to-br from-gray-900 via-purple-900 to-blue-900"
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <Truck className="absolute top-20 left-10 text-white/20 animate-bounce" size={40} style={{ animationDelay: '0s' }} />
        <Smartphone className="absolute top-32 right-20 text-white/20 animate-bounce" size={35} style={{ animationDelay: '1s' }} />
        <Zap className="absolute bottom-32 left-20 text-white/20 animate-bounce" size={30} style={{ animationDelay: '2s' }} />
      </div>
        ...
        <div className={`text-center transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-full px-8 py-4 mb-8">
            ...
            <div className="flex space-x-1">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse" style={{ animationDelay: `${i * 0.3}s` }} />
              ))}
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {[
              { icon: Zap, title: "Instant Sync", desc: "Real-time order synchronization" },
              { icon: Truck, title: "Unified Dashboard", desc: "Manage all platforms in one place" },
              { icon: CheckCircle, title: "Smart Analytics", desc: "Cross-platform performance insights" }
            ].map((feature, index) => (
              <div 
                key={index}
          ...
          <div className="mt-12 text-center">
            <p className="text-gray-400 text-lg">Expected Launch: Q2 2024</p>
